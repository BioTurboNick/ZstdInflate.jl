...
# allocating a table per FSE_Compressed-mode block.
...
mutable struct DecompressState
    ...
    # Reusable FSE decode tables, one per sequence field. These
...
# Create scratch vectors with a sizehint to avoid repeated reallocations during FSE table builds.
_new_fse_scratch(::Type{T}, size::Int) where {T} = sizehint!(T[], size)

DecompressState(dict::Union{Nothing, ZstdDict} = nothing) =
    DecompressState(
        dict === nothing ? INIT_REPEAT_OFFSETS : dict.rep,
        dict === nothing ? nothing             : dict.huffman,
        dict === nothing ? nothing             : dict.ll_tab,
        dict === nothing ? nothing             : dict.ml_tab,
        dict === nothing ? nothing             : dict.of_tab,
        dict === nothing ? EMPTY_DICT_CONTENT  : dict.content,
        UInt8[],
        ...
        _new_fse_table(_FSE_MAX_TABLE),
        _new_fse_table(_FSE_MAX_TABLE),
        _new_fse_scratch(Int, MAX_MATCH_LENGTH + 1),
        _new_fse_scratch(Int16, MAX_MATCH_LENGTH + 1),
...
function _run_sequences!(ll_tab::T1, of_tab::T2, ml_tab::T3,
    ...
    @inbounds for i in 1:num_seqs
        ...
        # Baselines come from the entries. The offset baseline is
